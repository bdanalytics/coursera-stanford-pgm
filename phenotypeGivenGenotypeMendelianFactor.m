...
%for asgP = 1:1,	
	for asgG = 1:phenotypeFactor.card(2),
		...
		if isDominant,
			if ((asgP == 1) && (asgG != 3)) || ((asgP == 2) && (asgG == 3)),
		%		printf('isDominant True: asgP: %d; asgG: %d; inx: %d\n', asgP, asgG, inx);
				phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [asgP, asgG], 1);
			endif;
		else,
			if ((asgP == 2) && (asgG != 3)) || ((asgP == 1) && (asgG == 3)),
			%if ((asgP == 1) && (asgG != 1)) || ((asgP == 2) && (asgG == 1)),				
				phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [asgP, asgG], 1);
			endif;
		endif;
	end;

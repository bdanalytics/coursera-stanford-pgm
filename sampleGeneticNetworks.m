...
% BBI
% indx = 19; assgn = [1 2 1 1 2]
computeSigmoid(alleleWeights{1}(1)*(0+1) + alleleWeights{1}(2)*(1+0) + alleleWeights{2}(1)*(1+0) + alleleWeights{2}(2)*(0+1))
ans = 0.52498

% indx = 11; assgn = [1 2 1 2 1]
computeSigmoid(alleleWeights{1}(1)*(0+0) + alleleWeights{1}(2)*(0+1) + alleleWeights{2}(1)*(1+0) + alleleWeights{2}(2)*(1+1))
ans = 0.014774

phenotypeFactorResults = constructSigmoidPhenotypeFactor(alleleWeights, geneCopyVarParentOneList, geneCopyVarParentTwoList, phenotypeVar)
all(phenotypeFactorResults.var == phenotypeFactorSigmoid.var)
all(phenotypeFactorResults.card == phenotypeFactorSigmoid.card)
all(phenotypeFactorResults.val == phenotypeFactorSigmoid.val)
phenotypeFactorResults.val(1) == phenotypeFactorSigmoid.val(1)
phenotypeFactorDiff = abs(phenotypeFactorResults.val - phenotypeFactorSigmoid.val)
max(phenotypeFactorDiff)

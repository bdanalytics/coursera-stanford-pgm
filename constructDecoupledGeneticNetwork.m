...
for personIx = 1:numPeople,
	parents = pedigree.parents(personIx, :);
	%printf('personIx: %d; parents: [%d %d]\n', personIx, parents(1), parents(2));

	if all(parents == [0 0]),
		factorList((numPeople * 0) + personIx) = childCopyGivenFreqsFactor(alleleFreqs, parents(1));
		factorList((numPeople * 0) + personIx).var = (numPeople * 0) + personIx;

		factorList((numPeople * 1) + personIx) = childCopyGivenFreqsFactor(alleleFreqs, parents(2));
		factorList((numPeople * 1) + personIx).var = (numPeople * 1) + personIx;

		%factorList((numPeople * 2) + personIx) = phenotypeGivenCopiesFactor(alphaList, numAlleles, (numPeople * 0) + personIx, (numPeople * 1) + personIx, (numPeople * 2) + personIx);		
	else	
		factorList((numPeople * 0) + personIx) = childCopyGivenParentalsFactor(numAlleles, (numPeople * 0) + personIx, parents(1), numPeople + parents(1));
		factorList((numPeople * 1) + personIx) = childCopyGivenParentalsFactor(numAlleles, (numPeople * 1) + personIx, parents(2), numPeople + parents(2));
		%factorList((numPeople * 2) + personIx) = phenotypeGivenCopiesFactor(alphaList, numAlleles, (numPeople * 0) + personIx, (numPeople * 1) + personIx, (numPeople * 2) + personIx);		
	end

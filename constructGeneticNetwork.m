...
for personIx = 1:numPeople,
	%factorList(personIx).var = [personIx];
	%factorList(personIx).card = [length(alphaList)];

	parents = pedigree.parents(personIx, :);
	if all(parents == [0, 0]),
		factorList(personIx) = genotypeGivenAlleleFreqsFactor(alleleFreqs, personIx);
	else
		factorList(personIx) = genotypeGivenParentsGenotypesFactor(numAlleles, personIx, parents(1), parents(2));

	end	
	%factorList(numPeople + personIx).var = [numPeople + personIx, personIx];
	%factorList(numPeople + personIx).card = [numAlleles, length(alphaList)];
	
	factorList(numPeople + personIx) = phenotypeGivenGenotypeFactor(alphaList, personIx, numPeople + personIx);

...
for asgP = 1:phenotypeFactor.card(1),
%for asgP = 1:1,	
	for asgG = 1:phenotypeFactor.card(2),
		inx = AssignmentToIndex([asgP, asgG], phenotypeFactor.card);
		%disp('asgP: '), disp(asgP);
		%printf('For loop: asgP: %d; asgG: %d; inx: %d\n', asgP, asgG, inx);
		if (asgP == 1),
		%		printf('isDominant True: asgP: %d; asgG: %d; inx: %d\n', asgP, asgG, inx);
			phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [asgP, asgG], alphaList(asgG));
		else,	
			phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [asgP, asgG], 1.0 - alphaList(asgG));
		endif;
	end;
end;

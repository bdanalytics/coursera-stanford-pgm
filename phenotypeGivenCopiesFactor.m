...
for indx = 1:prod(phenotypeFactor.card),
	...
	if numAlleles == 3,
		...
		printf('numAlleles: %d not supported\n', numAlleles);
	end

	% assgn(1) is prob of phenotype 1 (yes) or 2 (no)
	thisPhenotype = assgn(1);
	alleleOne = assgn(2);
	alleleTwo = assgn(3);
	thisGenotype = allelesToGenotypes(alleleOne, alleleTwo);

	if thisPhenotype == 1,
		phenotypeFactor.val(indx) = alphaList(thisGenotype);
	elseif thisPhenotype == 2,
		phenotypeFactor.val(indx) = 1.0 - alphaList(thisGenotype);
	else,	
		printf('thisPhenotype: %d not supported\n', thisPhenotype);
	end
end
